...
import { prepareStackedBinsFromArray, rebinHistogramData } from '../../../shared/utils/histogram.utils';
    ...
    if (this.enabled) {
      bars.on('mouseenter', (event: MouseEvent, d: StackedBin) => {
        ...
        if (this.lastHoveredBar) {
          d3.select(this.lastHoveredBar).attr('fill', displayColor).attr('opacity', 0.4);
        }

        // Highlight current bar with full color
        d3.select(currentBar).attr('fill', displayColor).attr('opacity', 0.9);
